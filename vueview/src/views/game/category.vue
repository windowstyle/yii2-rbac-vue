...
<script>
	import {
		API_FetchGameCategoryList,
		API_DoGameCategorySave,
		API_DoGameCategoryDelete
	...
	export default {
		...
		methods: {
			OnSearchList() {
				...
				// Todo:修改获取列表的api函数
				API_FetchGameCategoryList(this.searchModel).then(response => {
			...
			OnSelectRowDelete(id) {
				this.$confirm(this.$t('tips.comfirmDeleteWarning'), this.$t('table.warning'),
					...
					this.searchLoading = true;
					// Todo:修改删除的api函数
					API_DoGameCategoryDelete({ids}).then(() => {
						this.OnSearchList();
					}).catch(error => {
						this.$message.error(error.message);
						this.searchLoading = false;
			...
			OnSubmitFormDialog() {
				this.$refs['formDialogModel'].validate((isvalid) => {
					if(isvalid){
						...
						// Todo:修改保存的api函数
						API_DoGameCategorySave(postdata).then(response => {
